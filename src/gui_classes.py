...
class ThumbnailDownloader(ctk.CTkFrame):
    def __init__(self, master, isAudioDownloader : bool = False, **kwargs):
        ...
        ctk.CTkLabel(gridMaster, width=100, height = 10, text = "File name").grid(row = 1, column = 0)
        self.fileNameTextBox = ctk.CTkEntry(gridMaster, width=300, height = 10, corner_radius = 5, textvariable = self.fileName)
        self.fileNameTextBox.grid(row = 1, column = 1)
        self.fileNameTextBox.grid(row = 1, column = 1, pady = 5)
